...
All Java/JNI bridge sources in `proj/PROJECT/app/` are generated by
...
`proj/NAME/config.json` is developer-authored source configuration.
...
`proj/PROJECT/native/bridge.c` remains available for project-specific
...
`proj/PROJECT/app/` is builder output and must not be hand-edited. The
...
Run builds from project root with:
